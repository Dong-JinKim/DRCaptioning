...
function LM:__init(opt)
  ...
  ----------------------------------------------------------------------------------!!!!!!!!!!!!!!!!!!!!
  --local conv1 = nn.SpatialConvolution(D, W, 1, 1)
  --conv1.weight:normal(0, 0.01)
  --conv1.bias:zero()
  --self.image_encoder:add(conv1)-- 1*1 conv
  --self.image_encoder:add(nn.ELU(1,true))
  
  --local conv2 = nn.SpatialConvolution(W, W, 3, 3)
  --conv2.weight:normal(0, 0.01)
  --conv2.bias:zero()
  --self.image_encoder:add(conv2)-- 3*3 conv
  --self.image_encoder:add(nn.ELU(1,true))
  
  --self.image_encoder:add(nn.View(-1):setNumInputDims(3))
  
  --self.image_encoder:add(nn.Linear(W, W))
  --self.image_encoder:add(nn.ELU(1,true))
  --self.image_encoder:add(nn.Dropout(0.5))
  
  --self.image_encoder:add(nn.Linear(W, W))-- (D,W)!!!!
  ----------------------------------------------------------------------------------!!!!!!!!!!!!!!!!!!!
  ...
  self.lookup_table = nn.LookupTable(V + 2, W)----!!! W+64
  ...
  for i = 1, self.num_layers do
    ...
    self.rnn:add(nn.LSTM(input_dim, self.rnn_size))----!!!! ++64
  ...
  self.rnn:add(nn.Linear(H, V + 1))----!!!! H+64
...
function LM:backward(input, gradOutput, scale)
  ...
  local net_input = {input[1][1]:cuda(), self._gt_with_start}-----!!!
