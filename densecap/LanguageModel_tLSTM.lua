...
function LM:__init(opt)
  ...
  --self.image_encoder:add(nn.Linear(25088,W/2))
  --self.image_encoder:add(nn.ReLU(true))
  ----self.image_encoder:add(nn.Dropout(0.5))
  --self.image_encoder:add(nn.Linear(D/2,D/4))
  --self.image_encoder:add(nn.ReLU(true))
  ----self.image_encoder:add(nn.Dropout(0.5))
  ...
  --self.image_encoder2:add(nn.Linear(25088,W/2))
 ...
 -- self.image_encoder3:add(nn.View(-1):setNumInputDims(3))-- (B, 512,7,7)->(B,25088)
  --self.image_encoder3:add(nn.Linear(25088,W/2))
  ...
  if not(ATT) then
    --(1)-------------------------------------------------------- ------------------  not attention
    self.out:add(self.view_in)-- (B,T,3*H)->(B*T,3*H)
    self.out:add(nn.Linear(3*H, H))-- (B*T,3*H)->(B*T,H)
  ...
  self.out:add(nn.Linear(H, V + 1)) -- (BT, V)
...
function LM:sample(union_vectors, subj_vectors, obj_vectors,Masks)
  ...
  for t = 1, T do
    ...
    --concat/attention
    local scores = self.out:forward(  torch.cat( {self.rnn1:forward(wordvecs), self.rnn2:forward(wordvecs2),self.rnn3:forward(wordvecs3)} ,3)    ):view(N, -1)
    --sum
    --local scores = self.out:forward(  self.rnn1:forward(wordvecs)+ self.rnn2:forward(wordvecs2)+self.rnn3:forward(wordvecs3)     ):view(N, -1)--!!!
    --mul
    --local scores = self.out:forward(  torch.cmul(torch.cmul(self.rnn1:forward(wordvecs),self.rnn2:forward(wordvecs2)),self.rnn3:forward(wordvecs3) )    ):view(N, -1)--!!!
