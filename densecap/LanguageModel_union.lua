...
function LM:__init(opt)
  ...
  --self.image_encoder:add(nn.Linear(25088,W))
  --self.image_encoder:add(nn.ReLU(true))
  ...
  self.image_encoder:add(nn.Linear(W*3, W))----!!!222
  ...
  if S==0 then
    ...
    if S==6 then
      self.image_encoder_real:add(nn.Linear(W+S,W))--!!!666666
      self.image_encoder_real:add(nn.Dropout(self.dropout))--!!!666666
...
--dbg()
  if gradOutput == 0 then---if there is no box- pair detected!
    if ##input[3]==0 then--$$$
      ...
      --self.gradInput = { input[1].new(input[1]):zero() ,input[3].new(input[3]):zero() }--$$$
    end
  else
