...
function LM:__init(opt)
  ...
  local S = 10 --- size of spatial feature----------------------------!!!!!!!!!!!!!!! S = 10/64
  ...
  --self.image_encoder:add(nn.View(1, -1):setNumInputDims(3))
...
function LM:updateOutput(input)
  ...
  local gt_sequence = input[3]
  local Masks = input[2]:cuda() ------!!!!!
...
function LM:backward(input, gradOutput, scale)
  ...
  local net_input = {{input[1]:cuda(),input[2]:cuda()}, self._gt_with_start}-----------------------------!!!
  
  if gradOutput == 0 then---if there is no box- pair detected!
    self.gradInput = { input[1].new(input[1]):zero() , input[2].new(input[2]):zero() }
  else
    local tmp = self.net:backward(net_input, gradOutput, scale)
    self.gradInput ={tmp[1][1],tmp[1][2]}
  end
  
  self.gradInput[3] = input[3].new(#input[3]):zero()
