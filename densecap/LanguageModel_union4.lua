...
function LM:__init(opt)
  ...
  self.image_encoder:add(nn.Linear(512, W*2))----!!!222---!!!!!7777
  ...
  for i = 1, self.num_layers do
    ...
    self.rnn:add(nn.LSTM(input_dim*2+S, self.rnn_size))----!!!! 64----!!!!7777
  ...
  if self.ATT then---!!!!!!!!!!!!!!!!!!!!!!44444666
    ...
    att_branch:add(nn.Linear(H, 3))-- parts class !!!!!
  ...
  ---parallel:add(self.lookup_table)---!!!!777
  ------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  self.lookup_table2 = nn.LookupTable(4, W)
  self.parallel2 = nn.ParallelTable()---!!!!7777
  self.parallel2:add(self.lookup_table)
  
  self.parallel2:add(self.lookup_table2)
  
  local GT_input = nn.Sequential()
  GT_input:add(self.parallel2)
  GT_input:add(nn.JoinTable(2,2))
  parallel:add(GT_input)
  ------------------------------------------
...
function LM:updateOutput(input)
  ...
  if gt_sequence:nElement() > 0 then
    gt_parts = gt_sequence[{{},{16,30}}]---!!!!!666
    ...
    ---!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!777777777
    local gt_parts2 = gt_parts.new(N, T+1)-- fill with NuLL class
    gt_parts2[{{}, 1}]:fill(1)
    gt_parts2[{{}, {2, T + 1}}]:copy(gt_parts)
    mask = torch.eq(gt_parts2,0)
    gt_parts2[mask] = 4
    self._gt_with_start = { self._gt_with_start , gt_parts2}
    ----------------------------------------------------------
...
function LM:sample(image_vectors,Masks)
  ...
  local parts = torch.LongTensor(N, T):zero()---!!!777
  ...
  for t = 1, T do
    ...
    if t == 1 then
      ...
      cls = torch.LongTensor(N, 1):fill(1)---!!!777
    else
      -- On subsequent timesteps, feed previously sampled words
      words = seq[{{}, {t-1, t-1}}]
      cls = parts[{{}, {t-1, t-1}}]---!!!777
    end
    local wordvecs = self.lookup_table:forward(words)
    local clsvecs = self.lookup_table2:forward(cls)---!!!777
    
    local scores
    local pred
    if self.ATT then---6666!!!!
      pred = self.rnn:forward(torch.cat(wordvecs,clsvecs,3) )
      scores = pred[1]:view(N,-1)--!!! 444
      scores_part = pred[2]:view(N,-1)--!!! 777
      
    else
      scores = self.rnn:forward(torch.cat(wordvecs,clsvecs,3)):view(N, -1)
    end
    ...
    _, idx_part = torch.max(scores_part, 2)---!!!777
    
    parts[{{}, t}]:copy(idx_part)
    seq[{{}, t}]:copy(idx)
  end
