...
function layer:_forward_test(input)
  ...
  self.output = {roi_features, rpn_boxes_nms, empty, empty, empty}
  return self.output
...
function layer:_forward_train(input)
  ...
  self.output = {self.roi_features, self.roi_boxes, self.pos_target_boxes, self.pos_target_labels, self.nets.box_sampler_helper.pos_target_idx}-----!!!!
  return self.output
end
