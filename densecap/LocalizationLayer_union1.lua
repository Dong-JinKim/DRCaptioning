...
function layer:_forward_train(input)
  ...
  assert(gt_labels:dim() == 3 and gt_labels:size(1) == N and ((gt_labels:size(2) == B1/2) or(gt_labels:size(2) == B1)) ,-----!!!!!!!!!!!
  ...
  if not(##self.label_idx ==0) then
    --dbg()
    self.pos_target_labels = self.pos_target_labels:index(1,self.label_idx[{1,{}}]:typeAs(self.pos_target_labels)) --sample only if we have real union (might be danger) !!!!
  end
