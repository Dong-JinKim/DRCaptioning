...
require 'densecap.modules.Pairs_RPN' -- Pairs_RPN / Pairs_RPN1 / Pairs_RPN2
...
function layer:_forward_test(input)
  ...
  self:timeit('roi_pooling:forward_test', function()
    ...
    union_features = torch.cat({tmp_feats[{{rpn_boxes_nms:size(1)+1,-1},{}}] , 
                            tmp_feats:index(1,label_idx[{1,{}}]) ,tmp_feats:index(1,label_idx[{2,{}}])  },2) --!!!!222
  end)
  ...
  self.output = {roi_features, rpn_boxes_nms, empty, empty, union_features,Spatial}--!!!!!!
  return self.output
...
function layer:_forward_train(input)
  ...
  self:timeit('roi_pooling:forward', function()
    ...
    if tmp_feats:size(1) > num_pos + num_neg then
      ...
      self.union_features = torch.cat({tmp_feats[{{num_pos + num_neg+1,-1},{}}] , 
                            tmp_feats:index(1,self.label_idx[{1,{}}])     ,tmp_feats:index(1,self.label_idx[{2,{}}]) },2) --!!!!!!!222
  ...
  --self.union_features = self.nets.roi_pooling:forward{cnn_features[1], self.union_boxes}
  ...
  self.output = {self.roi_features, self.roi_boxes, self.pos_target_boxes, self.pos_target_labels, self.union_features,self.Spatial}-----!!!!
  return self.output
end
...
function layer:updateGradInput(input, gradOutput)
  ...
  --local grad_roi_features = torch.cat(gradOutput[1],gradOutput[5],1)
  local grad_roi_features
  if ##gradOutput[5] ==0 then
    grad_roi_features = gradOutput[1]------!!!!!!2222
  else
    grad_roi_features = torch.cat(gradOutput[1],gradOutput[5][{{},{1,512},{}}],1)------!!!!!!2222
  end
  ...
  self:timeit('roi_pooling:backward', function()
    local din = self.nets.roi_pooling:backward(
                    ...
                    grad_roi_features)
    grad_roi_boxes:add(din[2][{{1,self.roi_boxes:size(1)},{}}])--- only add grad from single box grad !!!!
