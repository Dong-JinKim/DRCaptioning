...
function layer:updateOutput(input)--------- we need to select only relevant pairs!
  ...
  local spatial = torch.CudaTensor(1,6):zero()---!!!!222222
  
  
  local RETIREVAL = false--whether is is for retrieval or not
  ...
  for i =1,B do---subj
    for j = 1,B do----obj
      ...
      elseif not( (torch.ceil(box_idxs[i]/2)==torch.ceil(box_idxs[j]/2) and box_idxs[i]%2==1 and box_idxs[j]%2==0) ) then--for training data
        if not(i==j) and RETIREVAL then
            continue = false---for retrieval, change to false !!!!!!!!!!!!!!!!!!!!!!
          else
            continue = true
          end
      else
