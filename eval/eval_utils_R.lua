  ...
  for d1=1,nd do -- for each "detection" in descending order of confidence
    for d2=1,nd do--!!!
      if not(d1==d2) then
        ...
        record.ov = ovmax
        record.candidate = text[(ii-1)*(nd-1) +  jj]---!!! originally ii
        
          
        
        record.references = {target_text[jmax]} -- will be nil if jmax stays -1 ---!!!!!
...
function DenseCaptioningEvaluator:evaluate(verbose)
  ...
  local results = {map = map, ap_breakdown = ap_results, detmap = detmap, det_breakdown = det_results, meteor = METEOR}
  return results
end
