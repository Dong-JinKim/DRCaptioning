...
function eval_utils.eval_split(kwargs)
  ...
  print(string.format('Recall: %f', 100 * ap_results.recall))
  print(string.format('METEOR: %f', 100 * ap_results.meteor))
...
function DenseCaptioningEvaluator:addResult(logprobs, boxes, text, target_boxes, target_text)
  ...
  local nd = boxes:size(1) -- number of pairs !!!!!!
  local nt = #target_text -- number of gt boxes pairs !!!!!
  --local used = torch.zeros(nt)
  
  for j=1,nt do --for all GT !!!
    ...
    for d1=1,nd do -- for each "detection" in descending order of confidence
      for d2=1,nd do--!!!
        if not(d1==d2) then
          local ii = d1--IX[d]--!!!! no ordering!
          local jj = d2--!!!
          
          local bb1 = boxes[ii] --box1
          local bb2 = boxes[jj] --box2
          ...
          if iw1>0 and ih1>0 and iw2>0 and ih2>0 then--if both boxes are intersected !!!
            ...
            local ov = (iw1*ih1/ua1 + iw2*ih2/ua2)/2 --- ave of intersection!!!
            if ov > ovmax then
              ovmax = ov
              imax = ii
              jmax = jj
    ...
    record.ov = ovmax
    record.candidate = target_text[j]---!!! originally ii
    
      
      
    record.references = {text[(imax-1)*(nd-1) +  jmax]} -- will be nil if jmax stays -1 ---!!!!!
...
function DenseCaptioningEvaluator:evaluate(verbose)
  ...
  for foo, min_overlap in pairs(min_overlaps) do
    for foo2, min_score in pairs(min_scores) do
      ...
      local n = #scores--ynElement()---!!!! num of box pairs!!!
      ...
      for i=1,n do
        ...
        if not r.references then ---!!!!!!
          tn[i] = 1 -- nothing aligned to this predicted box in the ground truth
        else
  ...
  local results = {recall = mrecall, ap_breakdown = ap_results, detmap = detmap, det_breakdown = det_results, meteor = METEOR}
  return results
end
