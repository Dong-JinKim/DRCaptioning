...
function eval_utils.eval_split(kwargs)
  ...
  utils.write_json('relcap_statistics_MTTS22v3_75.json', json_out)----!!!!333
...
function DenseCaptioningEvaluator:addResult(logprobs, boxes, text, target_boxes, target_text)
  ...
  table.insert(self.captions, text)---!!!!333
  ...
  for j=1,nt do --for all GT !!!
    ...
    record.references = reference--{text[(imax-1)*(nd-1) +  jmax]} -- will be nil if jmax stays -1 ---!!!!!
