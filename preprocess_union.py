...
def encode_boxes(data, original_heights, original_widths, image_size):
  ...
  for i, img in enumerate(data):
    ...
    for region in img['relationships']:
      ...
      #----------------------------------------------subject-----------------------------------------------------
      xs, ys = round(scale*(region['subject']['x']-1)+1), round(scale*(region['subject']['y']-1)+1)
      ws, hs = round(scale*region['subject']['w']), round(scale*region['subject']['h'])  
      #------------------------------------------------object--------------------------------------------------------
      xo, yo = round(scale*(region['object']['x']-1)+1), round(scale*(region['object']['y']-1)+1)
      wo, ho = round(scale*region['object']['w']), round(scale*region['object']['h'])  
...
if __name__ == '__main__':
  ...
  parser.add_argument('--region_data',
      default='data/visual-genome/1.0/region_descriptions.json',
      help='Input JSON file with regions and captions')
  parser.add_argument('--relationship_data',
      default='data/visual-genome/1.0/relationships_as_caption.json',
  ...
  parser.add_argument('--json_output',
      default='data/VG-regions-dicts_union.json',
      help='Path to output JSON file')
  parser.add_argument('--h5_output',
      default='data/VG-regions_union.h5',
