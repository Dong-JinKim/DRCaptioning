...
function run_image(model, img_path, opt, dtype)
  ...
  local tmp_captions={}
  local tmp_boxes = torch.CudaTensor(0)
  local tmp_scores = torch.CudaTensor(0)
  for i=1,scores:size(1) do--- [1,2,3,...] -> [1-2,1-3,...,2-1,2-3,....] make pairs
    for j=1,scores:size(1) do
      if not(i==j) then --and scores[i] >0.3 and scores[j] >0.3 then
        tmp_boxes = tmp_boxes:cat(boxes_xywh[{{i},{}}],1)
        tmp_boxes = tmp_boxes:cat(boxes_xywh[{{j},{}}],1)
        
        local tmp=nil
        if scores[i][1] < scores[j][1] then-- take score for either of the weak box (strict threshold)
          tmp= scores[{{i},{}}]
        else
          tmp= scores[{{j},{}}]
        end
        tmp_scores = tmp_scores:cat(tmp,1)
        tmp_scores = tmp_scores:cat(tmp,1)
      end
    end
  end
  
  for i=1,#captions do--- [1,2,3,...] -> [1,0,2,0,3,0,...]
    tmp_captions[2*i-1] = captions[i]
    tmp_captions[2*i] = "."
  end
  local out = {
    img = img,
    ...
    ----------------------------------------------------------------------------
    --boxes = tmp_boxes,---!!!!!!!!-------save "paired" of  boxes?
    --captions = tmp_captions,----!!!!!!!
    
    boxes = boxes_xywh,-----------------or just "single" box each?
    captions = captions,----!!!!!!!
    ----------------------------------------------------------------------------
...
function result_to_json(result)
  ...
  if not(result.transfer==nil) then
    out.transfer = result.transfer:totable()
  end
  if not(result.POS==nil) then
    out.POS = result.POS:totable()
  end
