...
local function lossFun()
  ...
  --if data.gt_labels:size(3) == 30 then----!!!!
  --  data.gt_labels2 = data.gt_labels[{{},{},{16,30}}]
  --  data.gt_labels = data.gt_labels[{{},{},{1,15}}]
  --end
...
while true do  
  ...
  if ((opt.eval_first_iteration == 1 or iter > 0) and iter % opt.save_checkpoint_every == 0) or (iter+1 == opt.max_iters) then
    ...
    if results.ap_results.map > best_val_score then------------------!!!!!!!!!
      best_val_score = results.ap_results.map-- (map or meteor_)
  ...
  if iter % 3 == 0 then collectgarbage() end
  if loss0 == nil then loss0 = losses.total_loss end
  --if losses.total_loss > loss0 * 1000 then
