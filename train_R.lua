...
require 'densecap.DenseCapModel_R'
require 'densecap.optim_updates'
local utils = require 'densecap.utils'
local opts = require 'train_opts'
local models = require 'models'
local eval_utils = require 'eval.eval_utils_R'
...
local params, grad_params, cnn_params, cnn_grad_params, D_params, D_grad_params = model:getParameters()
...
print('total number of parameters in D: ', D_grad_params:nElement())
-------------------------------------------------------------------------------
-- Loss function
-------------------------------------------------------------------------------
local loss_history = {}
...
local function lossFun()
  ...
  local losses, stats
  if current_model == 'D' then--- setting for GAN  
    losses, stats = model:forward_backward_D(data)-- train for D
  elseif current_model == 'G'    then
    losses, stats = model:forward_backward_G(data)-- train for G
  end
  ...
  if opt.losses_log_every > 0 and iter % opt.losses_log_every == 0 then
    ...
    if current_model == 'D' then
        D_loss[#D_loss] = losses_copy.captioning_loss
    elseif current_model == 'G'    then
        G_loss[#G_loss] = losses_copy.captioning_loss
    end
  end
...
while true do  
  ...
  for k=1, 1 do 
      ...
      -- Parameter update
      adam(D_params, D_grad_params, opt.learning_rate, opt.optim_beta1,
            opt.optim_beta2, opt.optim_epsilon, D_optim_state)-- 0,2,4,6 then train for D
      
      -- print loss and timing/benchmarks
      print(string.format('iter %d: %s (%s)', iter ,utils.build_loss_string(losses), current_model ))
  ...
  adam(params, grad_params, opt.learning_rate, opt.optim_beta1,
        opt.optim_beta2, opt.optim_epsilon, optim_state)-- 1,3,5,7 then train for G

  -- Make a step on the CNN if finetuning
  if opt.finetune_cnn_after >= 0 and iter >= opt.finetune_cnn_after then
    adam(cnn_params, cnn_grad_params, opt.learning_rate, opt.optim_beta1, 
        opt.optim_beta2, opt.optim_epsilon, cnn_optim_state)
  end
  ...
  if ((opt.eval_first_iteration == 1 or iter > 0) and iter % opt.save_checkpoint_every == 0) or (iter+1 == opt.max_iters) then
    ...
    if results.loss_results.captioning_loss < best_val_score then------------------!!!!!!!!!
      best_val_score = results.loss_results.captioning_loss
      ...
      if cudnn then
        ...
        cudnn.convert(model.nets.discriminator_model,nn)
      end
      ...
      if cudnn then
        ...
        cudnn.convert(model.nets.discriminator_model,cudnn)
      end
      ...
      params, grad_params, cnn_params, cnn_grad_params, D_params, D_grad_params = model:getParameters()
    end
  end
